<!DOCTYPE html>
<html lang="en">
	<head>
           <meta charset="UTF-8">
           <title>Litengine | Build websites in Liteyear</title>
           
           <base href="" target="_blank">
           <meta name="keywords" content="Xtraweb, CMS, Website Design Tool, Builf Frontend Apps " />
           <meta name="description" content="Xtraweb is a Lite CMS with A +400 professionally designed blocks allowing you to start building sites and pages instantly with Easy to use drag &amp; drop builder! In 10 minutes, Your Site is Ready!
Now You Build Efficiently and Swiftly." />
           <meta name="viewport" content="width=device-width,initial-scale=1.0,viewport-fit=cover">
		<link rel="stylesheet" href="css/fonts.css" />
		<link rel="stylesheet" href="css/bootstrap.weber.css" />
		<link rel="stylesheet" href="css/fx.css" />
		<link rel="stylesheet" href="css/owl.carousel.css" />
		<link rel="stylesheet" href="css/magnific-popup.css" />
		<link rel="stylesheet" href="css/aos.css" />
           <link rel="stylesheet" href="css/custom.css" />
		<link rel="stylesheet" href="css/home.css" />
		<link rel="stylesheet" href="css/preloader.css" />

       </head>
           <body class="light-page">
	<div id="preloader"><img src="images//uploaded/2/FP5f833WUAMolMG-removebg-preview.png" srcset="" alt="preloader image"/></div>
		<div id="wrap">
			<nav id="nav-menu-logo-social" class="navbar navbar-expand-lg light">
    			<div class="container-fluid">
        			<div class="row align-items-center">
           			<div class="col-3 hidden-lg">
                			<button class="navbar-toggler collapsed" type="button" data-toggle="collapse" data-target=".main-menu-collapse" aria-controls="navbarMenuContent" aria-expanded="false" aria-label="Toggle navigation" style=""><span class="icon-bar"></span><span class="icon-bar"></span><span class="icon-bar"></span></button>
            			</div>
            			<div class="col-6 col-lg-2 order-lg-2 text-center">
                			<a class="navbar-brand" href="#">
                    			<img src="images//uploaded/2/FP5f833WUAMolMG-removebg-preview.png" height="30" alt="" class="mw-100" srcset="" style="height: 50px;">
                			</a>
            			</div>
            			<div class="col-lg-5 order-lg-1 collapse navbar-collapse main-menu-collapse">
                			<ul class="navbar-nav" style="">
                    			<li class="nav-item active">
                        			<a class="nav-link" href="#">Home</a>
                    			</li>
                    			<li class="nav-item">
                        			<a class="nav-link smooth" href="showcase.html-" style="" target="_self">Showcase</a>
                    			</li>
                    			<li class="nav-item">
                        			<a class="nav-link smooth" href="templates.html-" style="" target="_self">Template</a>
                    			</li>
                    			<li class="nav-item">
                        			<a class="nav-link smooth" href="#price-4col-2" style="" target="_self">Pricing</a>
                    			</li><li class="nav-item">
                        			<a class="nav-link smooth" href="https://build.codecube.com.ng/dashboard" style="" target="_self">Login</a>
                    			</li>
                			</ul>
            			</div>
            			<div class="col-lg-5 order-lg-3 collapse navbar-collapse main-menu-collapse justify-content-lg-end">
                			<div class="d-inline-block inline-group navbar-text">
                    			<a href="#"><svg xmlns="http://www.w3.org/2000/svg" height="16" viewBox="0 0 16 16" width="16" class="icon svg-default"><path d="m16 2.57842783c-.5878969.26772602-1.2208722.44877814-1.8848389.52967377.6780537-.41603467 1.1983331-1.07571927 1.4434466-1.86156254-.6348535.38521729-1.3363855.66546287-2.0858132.81569761-.5972882-.65390635-1.4509597-1.06223667-2.3957269-1.06223667-1.81158659 0-3.28132891 1.50716263-3.28132891 3.36583604 0 .26387384.02817397.52004334.08452192.7665824-2.72817984-.14060431-5.14644597-1.47923438-6.76644949-3.51799687-.28267888.49885639-.44420966 1.07764536-.44420966 1.69399302 0 1.16720838.57944473 2.19766462 1.460351 2.80149272-.53718378-.01637174-1.04431532-.16949561-1.48758585-.41988685v.04141086c0 1.63139521 1.13165464 2.99217528 2.6342666 3.30034911-.27516582.07896954-.56535775.11845431-.86494101.11845431-.21130481 0-.41791395-.02022391-.61794917-.05970868.41791395 1.33670394 1.62939485 2.31034064 3.06626754 2.33634284-1.12320245.9033345-2.53941422 1.4416757-4.07677408 1.4416757-.26483536 0-.52685332-.0154087-.78323649-.0462261 1.45283794.9534128 3.17896343 1.5110148 5.03187181 1.5110148 6.03862179 0 9.33967249-5.12820509 9.33967249-9.57650171 0-.14638257-.0028174-.29276514-.0084522-.43625858.6414275-.47478031 1.198333-1.06705188 1.6369079-1.74214518" fill="#00aaec" fill-rule="evenodd"></path></svg></a>
                    			<a href="#"><svg xmlns="http://www.w3.org/2000/svg" height="16" viewBox="0 0 16 16" width="16" class="icon svg-default"><path d="m8.54611842 16h-7.66304125c-.48785295 0-.88307717-.3954695-.88307717-.8831324v-14.23379728c0-.48778708.3952863-.88307032.88307717-.88307032h14.23390773c.4876667 0 .8830151.39528324.8830151.88307032v14.23379728c0 .487725-.3954105.8831324-.8830151.8831324h-4.0772165v-6.19608178h2.0797387l.3114113-2.41472301h-2.39115v-1.54164808c0-.69911803.1941355-1.1755439 1.1966615-1.1755439l1.2786739-.00055875v-2.15974763c-.2211418-.0294274-.980176-.09517343-1.8632531-.09517343-1.84357263 0-3.10573228 1.12531866-3.10573228 3.19187953v1.78079226h-2.08507782v2.41472301h2.08507782z" fill="#4460a0" fill-rule="evenodd"></path></svg></a>
                    			<a href="#"><svg xmlns="http://www.w3.org/2000/svg" height="16" viewBox="0 0 16 16" width="16" class="icon svg-default"><path d="m11.8181925 13.6661767c-.0696184-.4120171-.4337038-2.4175436-1.3302758-4.87929916 2.2061504-.35275443 4.1168931.25210182 4.2580114.29819507-.3038748 1.90393319-1.3942494 3.54541709-2.9277356 4.58110409zm-3.81772211 1.1664413c-1.65390721 0-3.17045922-.587924-4.3530311-1.5662296.09031575.0733729.15334862.1194661.15334862.1194661s1.30957841-2.8558998 5.36155701-4.26785815c.01505262-.00564407.03104604-.00940678.04609866-.01411017.96148642 2.49750132 1.35755862 4.58957022 1.45916392 5.18690102-.81942734.3480511-1.72164407.5418308-2.66713711.5418308zm-6.83295113-6.83308834c0-.07243224.00376315-.1439238.00564473-.21541537.12136179.00282204 3.49691304.07901699 7.01452343-.97360221.19568413.38285614.38290116.7732377.55412477 1.16173791-.09031575.02445764-.17969071.05173731-.26906567.08183902-3.68695244 1.19089894-5.55724114 4.50773119-5.55724114 4.50773119s.00282237.002822.00282237.0037627c-1.08849298-1.2106532-1.75080849-2.81074722-1.75080849-4.56605324zm3.91744575-6.17743547c.09125655.12134752 1.35473629 1.834323 2.53730817 3.93579869-3.28053154.87200894-6.12830011.83720383-6.30799083.8343818.44311166-2.11652655 1.86464397-3.87089188 3.77068266-4.77018049zm1.27853237-.45528838c0 .00094068-.00094079.00188136-.00094079.00188136s-.0084671.00188135-.02069736.00376271c.00752632-.00188136.01411184-.00282204.02163815-.00564407zm6.14899752 1.5069669c-.0169342.02539832-.9897102 1.5069669-3.56935384 2.47304368-1.16751926-2.14568758-2.45075557-3.85019695-2.55330158-3.98565465.51649321-.12511024 1.05556535-.19283909 1.61063091-.19283909 1.72917035 0 3.30875521.64530543 4.51202451 1.70545006zm2.3181043 5.0580281c-.0987829-.0216356-2.4112425-.51925451-4.7585112-.22388147-.0489211-.11664413-.09690131-.23422894-.14770392-.35181375-.14205915-.33488153-.29540778-.66600035-.45440113-.9914751 2.69818305-1.10153448 3.79326155-2.68469634 3.80643265-2.70350991.9577233 1.16361926 1.5381902 2.64989124 1.5541836 4.27068023zm1.0075851-1.541772c-.1053684-.51361044-.263421-1.02063614-.4675722-1.50414487-.1994473-.47410194-.4487564-.93127168-.7375786-1.3583397-.2850591-.42330531-.6143353-.82215298-.9755983-1.18337351-.3622038-.36216121-.7601576-.69045799-1.1844535-.97642424-.4271182-.28972897-.8843417-.53712741-1.3584994-.7384326-.4835656-.20318655-.9897101-.36122053-1.50338099-.46563584-.52684189-.10817802-1.0687364-.16367805-1.61063092-.16367805-.5428353 0-1.08472981.05550003-1.6115717.16367805-.51367084.10441531-1.01981537.26244929-1.50338096.46563584-.4741577.20130519-.9313812.44870363-1.35944023.7384326-.42335509.28596625-.82130888.61426303-1.18351267.97642424-.36126301.36122053-.6895984.7600682-.97559829 1.18337351-.28882225.42706802-.53813135.88423776-.73757864 1.3583397-.20509202.48350873-.3622038.99053443-.46757218 1.50414487-.10724995.52583926-.16181572 1.06767006-.16181572 1.60950085 0 .54371215.05456577 1.08554295.16181572 1.61232289.10536838.51361045.26248016 1.01969545.46757218 1.50226355.19944729.4741019.44875639.933153.73757864 1.360221.28599989.4233053.61433528.8202716.97559829 1.1833735.36220379.3612206.76015758.6885767 1.18351267.9754836.42805903.2887883.88528253.5371274 1.35944023.7374919.48356559.2050679.98971012.3612205 1.50338096.4656358.52684189.1091187 1.0687364.1636781 1.6115717.1636781.54189452 0 1.08378903-.0545594 1.61063092-.1636781.51367089-.1044153 1.01981539-.2605679 1.50338099-.4656358.4741577-.2003645.9313812-.4487036 1.3584994-.7374919.4242959-.2869069.8222497-.614263 1.1844535-.9754836.361263-.3631019.6905392-.7600682.9755983-1.1833735.2888222-.427068.5381313-.8861191.7375786-1.360221.2041512-.4825681.3622038-.9886531.4675722-1.50226355.1072499-.52677994.1618157-1.06861074.1618157-1.61232289 0-.54183079-.0545658-1.08366159-.1618157-1.60950085z" fill="#ec4989" fill-rule="evenodd"></path></svg></a>
                    			<a href="#"><svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" height="16" viewBox="0 0 16 16" width="16" class="icon svg-default"><radialGradient id="a" cx="1.163903%" cy="98.836097%" r="139.775349%"><stop offset="0" stop-color="#ffc800"></stop><stop offset=".486454468" stop-color="#ff2a7b"></stop><stop offset="1" stop-color="#a000bc"></stop></radialGradient><path d="m8.00001588 0c2.17265372 0 2.44508882.00920925 3.29837392.04814217.8515067.03883765 1.4330552.17408667 1.9419142.37186335.5260707.20441369.9722115.47796031 1.4169867.92270379.4447435.44477523.7182901.89091598.9227355 1.41698671.197745.50885894.332994 1.09040748.3718316 1.94191421.0389329.85328506.0481422 1.12572022.0481422 3.29840565 0 2.17265372-.0092093 2.44508882-.0481422 3.29837392-.0388376.8515067-.1740866 1.4330552-.3718316 1.9419142-.2044454.5260707-.477992.9722115-.9227355 1.4169867-.4447752.4447435-.890916.7182901-1.4169867.9227355-.508859.197745-1.0904075.332994-1.9419142.3718316-.8532851.0389329-1.1257202.0481422-3.29837392.0481422-2.17268543 0-2.44512059-.0092093-3.29840565-.0481422-.85150673-.0388376-1.43305527-.1740866-1.94191421-.3718316-.52607073-.2044454-.97221148-.477992-1.41698671-.9227355-.44474348-.4447752-.7182901-.890916-.92270379-1.4169867-.19777668-.508859-.3330257-1.0904075-.37186335-1.9419142-.03893292-.8532851-.04814217-1.1257202-.04814217-3.29837392 0-2.17268543.00920925-2.44512059.04814217-3.29840565.03883765-.85150673.17408667-1.43305527.37186335-1.94191421.20441369-.52607073.47796031-.97221148.92270379-1.41698671.44477523-.44474348.89091598-.7182901 1.41698671-.92270379.50885894-.19777668 1.09040748-.3330257 1.94191421-.37186335.85328506-.03893292 1.12572022-.04814217 3.29840565-.04814217zm0 1.44143887c-2.13610246 0-2.38913467.0081613-3.23270238.04664963-.7799921.03556678-1.20358605.16589361-1.48548451.27545198-.37341939.14512515-.63991616.31848143-.91984575.59844276-.27996133.27992959-.45331761.54642636-.59844276.91984575-.10955837.28189846-.2398852.70549241-.27545198 1.48548451-.03848833.84356771-.04664963 1.09659992-.04664963 3.23270238 0 2.13607072.0081613 2.38910292.04664963 3.23267062.03556678.7799921.16589361 1.2035861.27545198 1.4854845.14512515.3734194.31851318.6399162.59844276.9198458.27992959.2799613.54642636.4533176.91984575.5984427.28189846.1095584.70549241.2398852 1.48548451.275452.84347244.0384883 1.09644114.0466496 3.23270238.0466496 2.13622952 0 2.38922992-.0081613 3.23267062-.0466496.7799921-.0355668 1.2035861-.1658936 1.4854845-.275452.3734194-.1451251.6399162-.3184814.9198458-.5984427.2799613-.2799296.4533176-.5464264.5984427-.9198458.1095584-.2818984.2398852-.7054924.275452-1.4854845.0384883-.8435677.0466496-1.0965999.0466496-3.23267062 0-2.13610246-.0081613-2.38913467-.0466496-3.23270238-.0355668-.7799921-.1658936-1.20358605-.275452-1.48548451-.1451251-.37341939-.3184814-.63991616-.5984427-.91984575-.2799296-.27996133-.5464264-.45331761-.9198458-.59844276-.2818984-.10955837-.7054924-.2398852-1.4854845-.27545198-.8435677-.03848833-1.0965999-.04664963-3.23267062-.04664963zm0 2.4504556c2.26884272 0 4.10808962 1.8392469 4.10808962 4.10812141 0 2.26884272-1.8392469 4.10808962-4.10808962 4.10808962-2.26887451 0-4.10812141-1.8392469-4.10812141-4.10808962 0-2.26887451 1.8392469-4.10812141 4.10812141-4.10812141zm0 6.77477223c1.47275033 0 2.66665082-1.19390049 2.66665082-2.66665082 0-1.47278209-1.19390049-2.66668255-2.66665082-2.66668255-1.47278209 0-2.66668255 1.19390046-2.66668255 2.66668255 0 1.47275033 1.19390046 2.66665082 2.66668255 2.66665082zm5.23041202-6.9370774c0 .53019901-.4298182.95998539-.9600172.95998539-.5301673 0-.9599854-.42978638-.9599854-.95998539 0-.53019902.4298181-.96001715.9599854-.96001715.530199 0 .9600172.42981813.9600172.96001715z" fill="url(#a)" fill-rule="evenodd"></path></svg></a>
                    			<a href="#"><svg xmlns="http://www.w3.org/2000/svg" height="16" viewBox="0 0 16 16" width="16" class="icon svg-default"><path d="m12.8690869.08769705c.7164521.3514996 3.6319254 6.64299232 1.1447499 12.12248595-1.9974115 4.3999949-7.01159273 4.4004949-9.5533504 2.8499967-2.17099248-1.3249984-5.48722652-5.49949361-1.81596268-10.59698771.155387-.19349978.52811912-.1759998.44403311.40849953-.05949945.41499952-.59204415 3.39449607.42436388 4.68899457.46419409.64749921.57286664.20099977.57286664.20099977s-.07916869-4.33549499 3.21690438-7.62699118c2.09231551-2.00699767 4.99402027-2.3279973 5.56639517-2.04699763" fill="#81b636" fill-rule="evenodd"></path></svg></a>
                			</div>
            			</div>
        			</div>
    			</div>
    			<div class="bg-wrap">
        			<div class="bg"></div>
    			</div>
			</nav><header id="header-text-text" class="pt-75 pb-75 pt-lg-250 pb-lg-250 text-center dark">
    			<div class="container">
        			<div class="row align-items-center">
            			<div class="col-lg text-lg-right" style="">
                			<h2 data-aos-once="true" data-aos-easing="ease-in-back" data-aos="zoom-in-up" style="color: rgb(238, 238, 238);"><strong class="">Liteweb Engine</strong></h2>
            			</div>
            			<hr class="sep-vertical-x2-lg ml-30 mr-30">
            			<div class="col-lg text-lg-left"><strong class="" style="">
                			<p class="text-secondary lead fx-text-glow" style="" data-aos="fade-left" data-aos-easing="ease-out-sine" data-aos-once="true">A Lite CMS with A +400 professionally designed blocks allowing you to start building sites and pages instantly with Easy to use drag &amp; drop builder! In 10 minutes, Your Site is Ready!<br>Now You Build Efficiently and Swiftly.</p>
            			</strong></div>
            			<div class="col-12" style="">
                			<a href="#" class="btn btn-outline-light btn-lg mt-50 mt-lg-150" style="">Get Started</a>
            			</div>
        			</div>
    			</div>
    			<div class="bg-wrap">
        			<div class="bg parallax" style="" data-rellax-speed="5"></div>
    			</div>
			</header><section id="benefits-3col-6" class="pt-lg-100 pb-lg-100 light text-center pt-50 pb-50">
    			<div class="container">
        			<div class="row align-items-center">
            			<div class="col-lg-12" style="z-index: 10;">
                			<h3 class="" style="overflow: visible;"><strong class="">What Can I Do WIth Xtraweb?</strong></h3>
                			<svg xmlns="http://www.w3.org/2000/svg" height="20" viewBox="0 0 40 20" width="40" class="svg-secondary mb-50"><path d="m0 8h40v4h-40z" fill-rule="evenodd"></path></svg>
            			</div>
            			<div class="col-lg-4 col-md-8 order-lg-1 ml-auto mr-auto text-lg-right">
                
                			<div class="content-box mb-50 d-lg-flex flex-lg-row-reverse">
                    			<div class="ml-lg-30">
                        			<svg xmlns="http://www.w3.org/2000/svg" height="48px" viewBox="0 0 17 17" width="48px" enable-background="new 0 0 64 64" class="mb-10 icon svg-secondary" srcset="" style="fill: rgb(0, 0, 0);"><path d="m8.5 0c-3.033 0-5.5 2.467-5.5 5.5v6c0 3.033 2.467 5.5 5.5 5.5s5.5-2.467 5.5-5.5v-6c0-3.033-2.467-5.5-5.5-5.5zm4.5 5.5v.5h-4v-4.949c2.244.252 4 2.139 4 4.449zm-5-4.449v4.949h-4v-.5c0-2.31 1.756-4.197 4-4.449zm.5 14.949c-2.481 0-4.5-2.019-4.5-4.5v-4.5h9v4.5c0 2.481-2.019 4.5-4.5 4.5z" class=""></path></svg>
                    			</div>
                    			<div class="" style="">
                        			<h4 class="mb-20" style="color: rgb(16, 209, 192);"><b class="">Drag and Drop</b></h4>
                        			<p class="text-right"><strong>Export your sites including all assets like Javascript files, CSS files and images. This enables users continue editting even after leaving our site</strong></p>
                    			</div>
                			</div><div class="content-box mb-50 d-lg-flex flex-lg-row-reverse">
                    			<div class="ml-lg-30">
                        			<svg xmlns="http://www.w3.org/2000/svg" height="48px" viewBox="0 0 20 20" width="48px" enable-background="new 0 0 64 64" class="mb-10 icon svg-secondary" style="fill: rgb(0, 0, 0);" srcset=""><path d="m5 15c-.128 0-.256-.049-.354-.146l-4-4c-.195-.195-.195-.512 0-.707l4-4c.195-.195.512-.195.707 0s.195.512 0 .707l-3.646 3.646 3.646 3.646c.195.195.195.512 0 .707-.098.098-.226.146-.354.146z"></path><path d="m15 15c-.128 0-.256-.049-.354-.146-.195-.195-.195-.512 0-.707l3.646-3.646-3.646-3.646c-.195-.195-.195-.512 0-.707s.512-.195.707 0l4 4c.195.195.195.512 0 .707l-4 4c-.098.098-.226.146-.354.146z"></path><path d="m7.5 15c-.091 0-.182-.025-.265-.076-.234-.146-.305-.455-.159-.689l5-8c.146-.234.455-.305.689-.159s.305.455.159.689l-5 8c-.095.152-.258.235-.424.235z"></path></svg>
                    			</div>
                    			<div class="" style="">
                        			<h4 class="mb-20" style="color: rgb(16, 209, 192);"><strong class="">Export Code</strong></h4>
                        			<p class="text-right" style=""><strong>Export your sites including all assets like Javascript files, CSS <strong>files </strong>and images. This enables users continue editting even after leaving our site</strong></p>
                    			</div>
                			</div>
                			<div class="content-box mb-50 mb-lg-0 d-lg-flex flex-lg-row-reverse">
                    			<div class="ml-lg-30">
                        			<svg xmlns="http://www.w3.org/2000/svg" enable-background="new 0 0 64 64" height="48px" viewBox="0 0 64 64" width="48px" class="mb-10 icon svg-secondary" style="fill: rgb(0, 0, 0);"><path d="m5.383 33.022c-.243.949-.383 1.945-.383 2.978 0 5.794 4.206 10 10 10v-2c-4.71 0-8-3.29-8-8 0-4.962 3.589-9 8-9h2.829l.153-.815c1.546-8.22 9.123-14.185 18.018-14.185 10.477 0 19 7.626 19 17v1h1c3.536 0 6 4.216 6 8 0 3.645-2.355 6-6 6v2c4.71 0 8-3.29 8-8 0-4.493-2.783-9.282-7.024-9.927-.537-10.048-9.74-18.073-20.976-18.073-4.097 0-7.938 1.156-11.156 3.157-2.647-3.251-6.59-5.157-10.844-5.157-7.72 0-14 6.28-14 14 0 2.682.761 5.288 2.201 7.538.85 1.327 1.939 2.509 3.182 3.484zm8.617-23.022c3.591 0 6.929 1.581 9.197 4.291-3.457 2.653-5.989 6.375-7.008 10.709h-1.189c-3.88 0-7.242 2.448-8.9 6.009-.849-.747-1.606-1.601-2.214-2.549-1.234-1.927-1.886-4.161-1.886-6.46 0-6.617 5.383-12 12-12z" class=""></path><path d="m17 44h37v2h-37z"></path><path d="m17 50h37v2h-37z"></path><path d="m17 56h37v2h-37z"></path></svg>
                    			</div>
                    			<div class="" style="">
                        			<h4 class="mb-20" style="color: rgb(16, 209, 192);"><strong class="">Live Site</strong></h4>
                        			<p class="" style=""><strong class="">Publish your sites/page directly to a live server using FTP, allowing you to use <span class="text-uppercase">XtraWeb </span>as a light CMS, you don't need to go through hosting stress</strong></p>
                    			</div>
                			</div>
            			</div>
            			<div class="col-lg-4 col-md-8 ml-auto mr-auto order-lg-3 text-lg-left">
                			<div class="content-box mb-50 d-lg-flex">
                    			<div class="mr-lg-30">
                        			<svg xmlns="http://www.w3.org/2000/svg" height="48px" viewBox="0 0 20 20" width="48px" enable-background="new 0 0 64 64" class="mb-10 icon svg-secondary" srcset="" style="fill: rgb(0, 0, 0);"><path d="m11.5 16h-3c-.276 0-.5-.224-.5-.5s.224-.5.5-.5h3c.276 0 .5.224.5.5s-.224.5-.5.5z"></path><path d="m16.5 11h-13c-.276 0-.5-.224-.5-.5s.224-.5.5-.5h13c.276 0 .5.224.5.5s-.224.5-.5.5z"></path><path d="m19.557 11.383-2.698-6.168c-.159-.364-.475-.699-.859-.925v-3.791c0-.133-.053-.26-.146-.354s-.221-.146-.354-.146h-11c-.133 0-.26.053-.354.146s-.146.221-.146.354v3.791c-.384.226-.699.56-.859.925l-2.698 6.168c-.248.568-.443 1.497-.443 2.117v4c0 .827.673 1.5 1.5 1.5h17c.827 0 1.5-.673 1.5-1.5v-4c0-.619-.194-1.549-.443-2.117zm-4.557-10.383v7h-10v-7zm-11 4.747v2.753c0 .276.224.5.5.5h11c.276 0 .5-.224.5-.5v-2.752l2.641 6.037c.03.069.06.148.089.233-.075-.012-.152-.018-.23-.018h-17c-.078 0-.155.006-.23.018.029-.085.058-.164.089-.233l2.641-6.037zm15 11.753c0 .276-.224.5-.5.5h-17c-.276 0-.5-.224-.5-.5v-4c0-.276.224-.5.5-.5h17c.276 0 .5.224.5.5z"></path><path d="m13.5 7h-7c-.276 0-.5-.224-.5-.5s.224-.5.5-.5h7c.276 0 .5.224.5.5s-.224.5-.5.5z"></path><path d="m13.5 5h-7c-.276 0-.5-.224-.5-.5s.224-.5.5-.5h7c.276 0 .5.224.5.5s-.224.5-.5.5z"></path><path d="m13.5 3h-7c-.276 0-.5-.224-.5-.5s.224-.5.5-.5h7c.276 0 .5.224.5.5s-.224.5-.5.5z"></path></svg>
                    			</div>
                    			<div class="">
                        			<h4 class="mb-20" style="color: rgb(16, 209, 192);"><strong class="">Blocks</strong></h4>
                        			<p class=""><strong>+400 professionally designed blocks allowing you to start building sites and pages instantly! These blocks are fully customizable.</strong></p>
                    			</div>
                			</div>
                			<div class="content-box mb-50 d-lg-flex">
                    			<div class="mr-lg-30">
                        			<svg xmlns="http://www.w3.org/2000/svg" height="48px" viewBox="0 0 17 17" width="48px" enable-background="new 0 0 64 64" class="mb-10 icon svg-secondary" srcset="" style="fill: rgb(0, 0, 0);"><path d="m17 3.5c0-2.273-4.38-3.5-8.5-3.5s-8.5 1.227-8.5 3.5c0 .149.024.318.08.5h-.08v9.5c0 2.273 4.38 3.5 8.5 3.5s8.5-1.227 8.5-3.5v-9.5h-.08c.056-.182.08-.351.08-.5zm-8.5-2.5c4.293 0 7.5 1.32 7.5 2.5 0 .398-.388.835-1.093 1.229-1.419.796-3.814 1.271-6.407 1.271s-4.988-.475-6.407-1.271c-.705-.394-1.093-.831-1.093-1.229 0-1.18 3.207-2.5 7.5-2.5zm0 15c-4.293 0-7.5-1.32-7.5-2.5v-1.633c.175.135.367.271.604.403 1.562.874 4.14 1.396 6.896 1.396.345 0 .687-.008 1.023-.024 2.361-.111 4.505-.607 5.872-1.372 0-.001.001-.001.001-.001.236-.133.43-.268.604-.403v1.634c0 1.18-3.207 2.5-7.5 2.5zm6.407-4.603c-1.42.795-3.814 1.27-6.407 1.27-.324 0-.646-.008-.962-.022-2.214-.102-4.203-.552-5.445-1.247-.704-.396-1.093-.833-1.093-1.231v-1.634c.175.135.368.271.605.403 1.561.874 4.139 1.396 6.895 1.396s5.334-.522 6.895-1.396c.237-.132.43-.268.605-.403v1.634c0 .398-.389.835-1.093 1.23zm-.001-3.334c-1.419.795-3.813 1.27-6.406 1.27s-4.987-.475-6.406-1.27c-.705-.394-1.094-.832-1.094-1.23v-1.634c.175.136.367.271.604.404 1.562.875 4.139 1.397 6.896 1.397s5.334-.522 6.896-1.397c.237-.133.429-.268.604-.404v1.634c0 .398-.389.836-1.094 1.23z" class=""></path></svg>
                    			</div>
                    			<div class="" style="">
                        			<h4 class="mb-20" style="color: rgb(16, 209, 192);"><strong class="">Sub Domain</strong></h4>
                        			<p class=""><strong class="">In our work we try to use only the most convenient and interesting solutions. We want the template you downloaded look unique.</strong></p>
                    			</div>
                			</div>
                			<div class="content-box mb-50 mb-lg-0 d-lg-flex">
                    			<div class="mr-lg-30">
                        			<svg xmlns="http://www.w3.org/2000/svg" height="48px" viewBox="0 0 17 17" width="48px" enable-background="new 0 0 64 64" class="mb-10 icon svg-secondary" srcset="" style="fill: rgb(0, 0, 0);"><path d="m8.5 0c-4.687 0-8.5 3.813-8.5 8.5s3.813 8.5 8.5 8.5 8.5-3.813 8.5-8.5-3.813-8.5-8.5-8.5zm0 16c-4.136 0-7.5-3.364-7.5-7.5s3.364-7.5 7.5-7.5 7.5 3.364 7.5 7.5-3.364 7.5-7.5 7.5zm4.5-7c0 2.481-2.019 4.5-4.5 4.5s-4.5-2.019-4.5-4.5c0-.276.224-.5.5-.5s.5.224.5.5c0 1.93 1.57 3.5 3.5 3.5s3.5-1.57 3.5-3.5c0-.276.224-.5.5-.5s.5.224.5.5zm-8.5-3.5c0-.552.448-1 1-1s1 .448 1 1-.448 1-1 1-1-.448-1-1zm6 0c0-.552.448-1 1-1s1 .448 1 1-.448 1-1 1-1-.448-1-1z"></path></svg>
                    			</div>
                    			<div class="" style="">
                        			<h4 class="mb-20" style="color: rgb(16, 209, 192);"><b class="">Many More...</b></h4>
                        			<p class="" style=""><strong>Everything combined in a beautifully designed interface! with asthonishing<br>
Clear and concise documentation and guide videos</strong></p>
                    			</div>
                			</div>
            			</div>
            			<div class="col-lg-4 order-lg-2">
                 			<div class="video-iframe embed-responsive embed-responsive-16by9 mb-50">
                    			<iframe class="embed-responsive-item" src="https://player.vimeo.com/video/108425305?title=0&amp;byline=0&amp;portrait=0" allowfullscreen="">
                    			</iframe>
                			</div>
            			</div>
        			</div>
    			</div>
    			<div class="bg-wrap">
        			<div class="bg"></div>
    			</div>
			</section><section id="testimonial-single-3" class="pt-75 light">
    			<div class="container">
        			<div class="row align-items-center">
            			<div class="col-md-6 order-md-2">
                			<div class="content-box mb-50 blockquote">
                    			<svg xmlns="http://www.w3.org/2000/svg" height="48px" viewBox="0 0 16 16" width="48px" class="icon svg-secondary mb-30" style="fill: rgb(238, 238, 238);"><g fill-rule="evenodd" class=""><path d="m0 10.5c0 1.9329966 1.56700338 3.5 3.5 3.5s3.5-1.5670034 3.5-3.5c0-1.93299662-1.56700338-3.5-3.5-3.5-.36702208 0-.72084959.05649267-1.05326142.16125692.66578014-1.28396861 2.00706995-2.16125692 3.55326142-2.16125692.34529957 0 .68038008.043753 1 .12601749v-2.04305905c-.32520847-.05455964-.65928779-.08295844-1-.08295844-3.3137085 0-6 2.6862915-6 6h.16840779-.16840779z"></path><path d="m9 10.5c0 1.9329966 1.5670034 3.5 3.5 3.5s3.5-1.5670034 3.5-3.5c0-1.93299662-1.5670034-3.5-3.5-3.5-.3670221 0-.7208496.05649267-1.0532614.16125692.6657801-1.28396861 2.0070699-2.16125692 3.5532614-2.16125692.3452996 0 .6803801.043753 1 .12601749v-2.04305905c-.3252085-.05455964-.6592878-.08295844-1-.08295844-3.3137085 0-6 2.6862915-6 6h.16840779-.16840779z" class=""></path></g></svg>
                    			<h4 class="mb-30" style="color: rgb(238, 238, 238);"><strong class="">In our work we try to use only the most modern, convenient and interesting solutions</strong></h4>
                    			<p class="blockquote-footer" style="color: rgb(238, 238, 238);">Author</p>
                			</div>
            			</div>
            			<div class="col-md-5 order-md-1 mr-auto">
                			<div class="content-box d-inline-block">
                    			<img class="mw-100" alt="img" src="images/isolated-5.png" srcset="">
                			</div>
            			</div>
        			</div>
    			</div>
    			<div class="bg-wrap">
        			<div class="bg"></div>
    			</div>
			</section><section id="desc-text-img" class="pt-75 pb-75 pt-lg-150 pb-lg-150 dark">
    			<div class="container">
        			<div class="row align-items-center">
            			<div class="col-md-6 col-lg-5 mr-auto" style="">
                			<h2 class="mb-50" style="color: rgb(16, 209, 192);"><b class="">XtraWeb<br></b>Swift &amp; Accurate</h2>
                			<p class="mb-50" style="color: rgb(0, 0, 0); border-radius: 0px 1px 0px 0px;">Create a unique looking landing pages, Websites in under 30 minutes easily with our 120+ pre-built blocks and templates. Xtraweb also comes with a SEO tool and even beautiful form builder.</p>
                			<a class="btn btn-info" href="#" style="background-color: rgb(16, 209, 192);">Details</a>
            			</div>
            			<div class="col-md-6">
                			<div class="content-box d-inline-block">
                    			<img alt="img" class="mw-100 mt-50 mt-lg-0 shadow" src="images/uploaded/2/WhatsApp_Image_2022-04-13_at_7.11.38_AM.jpeg" srcset="" style="box-shadow: rgb(162, 170, 177) 0px 0px 0px;">
                    			<img alt="img" class="position-absolute l-50 t-100 centered" src="images/bg-pattern-3-light.png" height="200px" width="200px" style="">
                			</div>
            			</div>
        			</div>
    			</div>
    			<div class="bg-wrap">
        			<div class="bg"></div>
    			</div>
			</section><section id="testimonial-5col" class="pt-75 pb-50 text-center light">
    			<div class="container-fluid">
        			<div class="row">
            			<div class="col-12 text-center">
                			<h3 class="" style="color: rgb(238, 238, 238);"><strong class="">Testimonials</strong></h3>
                			<svg xmlns="http://www.w3.org/2000/svg" height="20" viewBox="0 0 40 20" width="40" class="mb-50 svg-secondary"><path d="m0 8h40v4h-40z" fill-rule="evenodd"></path></svg>
            			</div>
            			<div class="col-lg col-md-4">
                			<div class="content-box bg-default padding mb-50">
                    			<img src="images/avatar-1.jpg" alt="photo" height="50px" class="mb-30 rounded-circle">
                    			<p class="">In our work we try to use only the most modern, convenient and interesting solutions</p>
                    			<p class="text-secondary small blockquote-footer">Author</p>
                			</div>
            			</div>
            			<div class="col-lg col-md-4">
                			<div class="content-box bg-default padding mb-50">
                    			<img src="images/avatar-2.jpg" alt="photo" height="50px" class="mb-30 rounded-circle">
                    			<p class="">In our work we try to use only the most modern, convenient and interesting solutions</p>
                    			<p class="text-secondary small blockquote-footer">Author</p>
                			</div>
            			</div>
            			<div class="col-lg col-md-4">
                			<div class="content-box bg-default padding mb-50">
                    			<img src="images/avatar-3.jpg" alt="photo" height="50px" class="mb-30 rounded-circle">
                    			<p class="">In our work we try to use only the most modern, convenient and interesting solutions</p>
                    			<p class="text-secondary small blockquote-footer">Author</p>
                			</div>
            			</div>
            			<div class="col-lg col-md-4">
                			<div class="content-box bg-default padding mb-50">
                    			<img src="images/avatar-1.jpg" alt="photo" height="50px" class="mb-30 rounded-circle">
                    			<p class="">In our work we try to use only the most modern, convenient and interesting solutions</p>
                    			<p class="text-secondary small blockquote-footer">Author</p>
                			</div>
            			</div>
            			<div class="col-lg col-md-4">
                			<div class="content-box bg-default padding mb-50">
                    			<img src="images/avatar-2.jpg" alt="photo" height="50px" class="mb-30 rounded-circle">
                    			<p class="">In our work we try to use only the most modern, convenient and interesting solutions</p>
                    			<p class="text-secondary small blockquote-footer">Author</p>
                			</div>
            			</div>
        			</div>
    			</div>
    			<div class="bg-wrap">
        			<div class="bg"></div>
    			</div>
			</section><section id="testimonial-single-4" class="dark">
    			<div class="container">
        			<div class="row align-items-center">
            			<div class="col-md-7">
                			<div class="content-box blockquote mt-50 mb-50">
                    			<svg xmlns="http://www.w3.org/2000/svg" height="48px" viewBox="0 0 16 16" width="48px" class="icon mb-30 svg-default" style="fill: rgb(0, 0, 0);"><g fill-rule="evenodd" class=""><path d="m0 10.5c0 1.9329966 1.56700338 3.5 3.5 3.5s3.5-1.5670034 3.5-3.5c0-1.93299662-1.56700338-3.5-3.5-3.5-.36702208 0-.72084959.05649267-1.05326142.16125692.66578014-1.28396861 2.00706995-2.16125692 3.55326142-2.16125692.34529957 0 .68038008.043753 1 .12601749v-2.04305905c-.32520847-.05455964-.65928779-.08295844-1-.08295844-3.3137085 0-6 2.6862915-6 6h.16840779-.16840779z" class=""></path><path d="m9 10.5c0 1.9329966 1.5670034 3.5 3.5 3.5s3.5-1.5670034 3.5-3.5c0-1.93299662-1.5670034-3.5-3.5-3.5-.3670221 0-.7208496.05649267-1.0532614.16125692.6657801-1.28396861 2.0070699-2.16125692 3.5532614-2.16125692.3452996 0 .6803801.043753 1 .12601749v-2.04305905c-.3252085-.05455964-.6592878-.08295844-1-.08295844-3.3137085 0-6 2.6862915-6 6h.16840779-.16840779z"></path></g></svg>
                    			<h4 class="mb-30" style="color: rgb(0, 0, 0);">"Professional design, development and implementation.. very user-friendly. I recommend using this product for building, hosting, or exporting of sites with ease"<br><br>~BerthJone Edward</h4>
                    
                			</div>
            			</div>
            			<div class="col-md-4 mr-auto">
                			<div class="content-box d-inline-block">
                    			<img class="mw-100 mt--50" alt="img" src="images//uploaded/2/WhatsApp_Image_2022-04-14_at_6.19.44_AM-removebg-preview.png" srcset="">
                			</div>
            			</div>
        			</div>
    			</div>
    			<div class="bg-wrap">
        			<div class="bg"></div>
    			</div>
			</section><section id="price-4col-2" class="pt-75 pb-75 pt-lg-100 pb-lg-100 text-center light">
    			<div class="container">
        			<div class="row">
            			<div class="col-12">
                			<h3 class="" style="color: rgb(255, 255, 255);"><strong class="">Prices</strong></h3>
                			<svg xmlns="http://www.w3.org/2000/svg" height="20" viewBox="0 0 40 20" width="40" class="mb-50 svg-secondary"><path d="m0 8h40v4h-40z" fill-rule="evenodd"></path></svg>
            			</div>
            			<div class="col-lg-3 col-md-6">
                			<div class="content-box bg-default mb-50 padding-x2" style="background-color: rgb(244, 245, 246);">
                    			<h4 class="" style="">NewBie</h4>
                    			<h3 class="mb-20">FREE</h3>
                    			<ul class="d-inline-block padding-list list-unstyled text-left mb-30">
                        			<li class=""><svg xmlns="http://www.w3.org/2000/svg" height="16" viewBox="0 0 16 16" width="16" class="icon svg-secondary"><path d="m10.0710678 12.0710678h-5.99999999v-2h5.99999999v-9.99999999h2v11.99999999z" fill-rule="evenodd" transform="matrix(.70710678 .70710678 -.70710678 .70710678 6.656854 -3.928932)"></path></svg> Benefit description</li>
                        			<li class=""><svg xmlns="http://www.w3.org/2000/svg" height="16" viewBox="0 0 16 16" width="16" class="icon svg-secondary"><path d="m10.0710678 12.0710678h-5.99999999v-2h5.99999999v-9.99999999h2v11.99999999z" fill-rule="evenodd" transform="matrix(.70710678 .70710678 -.70710678 .70710678 6.656854 -3.928932)"></path></svg> Benefit description</li>
                        			<li class=""><svg xmlns="http://www.w3.org/2000/svg" height="16" viewBox="0 0 16 16" width="16" class="icon svg-secondary"><path d="m10.0710678 12.0710678h-5.99999999v-2h5.99999999v-9.99999999h2v11.99999999z" fill-rule="evenodd" transform="matrix(.70710678 .70710678 -.70710678 .70710678 6.656854 -3.928932)"></path></svg> Benefit description</li>
                        			<li class=""><svg xmlns="http://www.w3.org/2000/svg" height="16" viewBox="0 0 16 16" width="16" class="icon svg-secondary"><path d="m10.0710678 12.0710678h-5.99999999v-2h5.99999999v-9.99999999h2v11.99999999z" fill-rule="evenodd" transform="matrix(.70710678 .70710678 -.70710678 .70710678 6.656854 -3.928932)"></path></svg> Benefit description</li>
                        			<li class=""><svg xmlns="http://www.w3.org/2000/svg" height="16" viewBox="0 0 16 16" width="16" class="icon svg-secondary"><path d="m10.0710678 12.0710678h-5.99999999v-2h5.99999999v-9.99999999h2v11.99999999z" fill-rule="evenodd" transform="matrix(.70710678 .70710678 -.70710678 .70710678 6.656854 -3.928932)"></path></svg> Benefit description</li>
                    			</ul>
                    			<a href="#" class="btn btn-block btn-secondary"><span>Try now</span></a>
                			</div>
            			</div>
            			<div class="col-lg-3 col-md-6">
                			<div class="content-box bg-default mb-50 padding-x2" style="background-color: rgb(244, 245, 246);">
                    			<h4 class="" style="">Starter</h4>
                    			<h3 class="mb-20"><strong class="">$9.99</strong></h3>
                    			<ul class="d-inline-block padding-list list-unstyled text-left mb-30">
                        			<li class=""><svg xmlns="http://www.w3.org/2000/svg" height="16" viewBox="0 0 16 16" width="16" class="icon svg-secondary"><path d="m10.0710678 12.0710678h-5.99999999v-2h5.99999999v-9.99999999h2v11.99999999z" fill-rule="evenodd" transform="matrix(.70710678 .70710678 -.70710678 .70710678 6.656854 -3.928932)"></path></svg> Benefit description</li>
                        			<li class=""><svg xmlns="http://www.w3.org/2000/svg" height="16" viewBox="0 0 16 16" width="16" class="icon svg-secondary"><path d="m10.0710678 12.0710678h-5.99999999v-2h5.99999999v-9.99999999h2v11.99999999z" fill-rule="evenodd" transform="matrix(.70710678 .70710678 -.70710678 .70710678 6.656854 -3.928932)"></path></svg> Benefit description</li>
                        			<li class=""><svg xmlns="http://www.w3.org/2000/svg" height="16" viewBox="0 0 16 16" width="16" class="icon svg-secondary"><path d="m10.0710678 12.0710678h-5.99999999v-2h5.99999999v-9.99999999h2v11.99999999z" fill-rule="evenodd" transform="matrix(.70710678 .70710678 -.70710678 .70710678 6.656854 -3.928932)"></path></svg> Benefit description</li>
                        			<li class=""><svg xmlns="http://www.w3.org/2000/svg" height="16" viewBox="0 0 16 16" width="16" class="icon svg-secondary"><path d="m10.0710678 12.0710678h-5.99999999v-2h5.99999999v-9.99999999h2v11.99999999z" fill-rule="evenodd" transform="matrix(.70710678 .70710678 -.70710678 .70710678 6.656854 -3.928932)"></path></svg> Benefit description</li>
                        			<li class=""><svg xmlns="http://www.w3.org/2000/svg" height="16" viewBox="0 0 16 16" width="16" class="icon svg-secondary"><path d="m10.0710678 12.0710678h-5.99999999v-2h5.99999999v-9.99999999h2v11.99999999z" fill-rule="evenodd" transform="matrix(.70710678 .70710678 -.70710678 .70710678 6.656854 -3.928932)"></path></svg> Benefit description</li>
                    			</ul>
                    			<a href="#" class="btn btn-block btn-secondary"><span>Buy now</span></a>
                			</div>
            			</div>
            			<div class="col-lg-3 col-md-6">
                			<div class="content-box bg-default mb-50 padding-x2" style="background-color: rgb(242, 240, 237);">
                    			<img class="position-absolute l-100 t-25 centered" src="images/stamp-2-primary.png" alt="Best" width="80">
                    			<h4 class="" style="">Ultimate</h4>
                    			<h3 class="mb-20" style=""><strong class="">$9.99</strong></h3>
                    			<ul class="d-inline-block padding-list list-unstyled text-left mb-30">
                        			<li class=""><svg xmlns="http://www.w3.org/2000/svg" height="16" viewBox="0 0 16 16" width="16" class="icon svg-primary"><path d="m10.0710678 12.0710678h-5.99999999v-2h5.99999999v-9.99999999h2v11.99999999z" fill-rule="evenodd" transform="matrix(.70710678 .70710678 -.70710678 .70710678 6.656854 -3.928932)"></path></svg> Benefit description</li>
                        			<li class=""><svg xmlns="http://www.w3.org/2000/svg" height="16" viewBox="0 0 16 16" width="16" class="icon svg-primary"><path d="m10.0710678 12.0710678h-5.99999999v-2h5.99999999v-9.99999999h2v11.99999999z" fill-rule="evenodd" transform="matrix(.70710678 .70710678 -.70710678 .70710678 6.656854 -3.928932)"></path></svg> Benefit description</li>
                        			<li class=""><svg xmlns="http://www.w3.org/2000/svg" height="16" viewBox="0 0 16 16" width="16" class="icon svg-primary"><path d="m10.0710678 12.0710678h-5.99999999v-2h5.99999999v-9.99999999h2v11.99999999z" fill-rule="evenodd" transform="matrix(.70710678 .70710678 -.70710678 .70710678 6.656854 -3.928932)"></path></svg> Benefit description</li>
                        			<li class=""><svg xmlns="http://www.w3.org/2000/svg" height="16" viewBox="0 0 16 16" width="16" class="icon svg-primary"><path d="m10.0710678 12.0710678h-5.99999999v-2h5.99999999v-9.99999999h2v11.99999999z" fill-rule="evenodd" transform="matrix(.70710678 .70710678 -.70710678 .70710678 6.656854 -3.928932)"></path></svg> Benefit description</li>
                        			<li class=""><svg xmlns="http://www.w3.org/2000/svg" height="16" viewBox="0 0 16 16" width="16" class="icon svg-primary"><path d="m10.0710678 12.0710678h-5.99999999v-2h5.99999999v-9.99999999h2v11.99999999z" fill-rule="evenodd" transform="matrix(.70710678 .70710678 -.70710678 .70710678 6.656854 -3.928932)"></path></svg> Benefit description</li>
                    			</ul>
                    			<a href="#" class="btn btn-block btn-primary"><span>Buy now</span></a>
                			</div>
            			</div>
            			<div class="col-lg-3 col-md-6">
                			<div class="content-box bg-default mb-50 padding-x2" style="background-color: rgb(244, 245, 246); z-index: 10;">
                    			<h4 class="" style="">Pro</h4>
                    			<h3 class="mb-20" style="overflow: visible;"><strong class="">$49.99</strong></h3>
                    			<ul class="d-inline-block padding-list list-unstyled text-left mb-30">
                        			<li class=""><svg xmlns="http://www.w3.org/2000/svg" height="16" viewBox="0 0 16 16" width="16" class="icon svg-secondary"><path d="m10.0710678 12.0710678h-5.99999999v-2h5.99999999v-9.99999999h2v11.99999999z" fill-rule="evenodd" transform="matrix(.70710678 .70710678 -.70710678 .70710678 6.656854 -3.928932)"></path></svg> Benefit description</li>
                        			<li class=""><svg xmlns="http://www.w3.org/2000/svg" height="16" viewBox="0 0 16 16" width="16" class="icon svg-secondary"><path d="m10.0710678 12.0710678h-5.99999999v-2h5.99999999v-9.99999999h2v11.99999999z" fill-rule="evenodd" transform="matrix(.70710678 .70710678 -.70710678 .70710678 6.656854 -3.928932)"></path></svg> Benefit description</li>
                        			<li class=""><svg xmlns="http://www.w3.org/2000/svg" height="16" viewBox="0 0 16 16" width="16" class="icon svg-secondary"><path d="m10.0710678 12.0710678h-5.99999999v-2h5.99999999v-9.99999999h2v11.99999999z" fill-rule="evenodd" transform="matrix(.70710678 .70710678 -.70710678 .70710678 6.656854 -3.928932)"></path></svg> Benefit description</li>
                        			<li class=""><svg xmlns="http://www.w3.org/2000/svg" height="16" viewBox="0 0 16 16" width="16" class="icon svg-secondary"><path d="m10.0710678 12.0710678h-5.99999999v-2h5.99999999v-9.99999999h2v11.99999999z" fill-rule="evenodd" transform="matrix(.70710678 .70710678 -.70710678 .70710678 6.656854 -3.928932)"></path></svg> Benefit description</li>
                        			<li class=""><svg xmlns="http://www.w3.org/2000/svg" height="16" viewBox="0 0 16 16" width="16" class="icon svg-secondary"><path d="m10.0710678 12.0710678h-5.99999999v-2h5.99999999v-9.99999999h2v11.99999999z" fill-rule="evenodd" transform="matrix(.70710678 .70710678 -.70710678 .70710678 6.656854 -3.928932)"></path></svg> Benefit description</li>
                    			</ul>
                    			<a href="#" class="btn btn-block btn-secondary"><span>Buy now</span></a>
                			</div>
            			</div>
            			<div class="col-12">
                			<p class="text-secondary" style="color: rgb(238, 238, 238);">*In our work we try to use only the most modern, convenient and interesting solutions.</p>
            			</div>
        			</div>
    			</div>
    			<div class="bg-wrap">
        			<div class="bg"></div>
    			</div>
			</section><section id="action-text-btn" class="pt-50 pb-50 pt-lg-100 pb-lg-100 dark text-center">
    			<div class="container">
        			<div class="row align-items-center justify-content-center">
            			<div class="col-md-auto mw-100">
                			<h2 class=""><strong class="">Let's Gist You From Here</strong></h2>
            			</div>
            			<div class="col-md-auto mw-100">
                			<a href="#" class="btn btn-lg btn-success mb-30 mt-30" data-toggle="modal" data-target="#popup-subscribe">Subscribe</a>
            			</div>
        			</div>
    			</div>
    			<div class="bg-wrap">
        			<div class="bg"></div>
    			</div>
			</section><section id="gallery-fluid-carousel-4" class="section-carousel gallery carousel-nav-none carousel-dots-left-bottom dark text-right">
    			<div class="container-fluid">
        			<div class="row no-gutters">
            			<div class="col-12">
                			<div class="carousel-3item-nogutters owl-drag"><div class="item gallery-item padding dark gallery-style-4">
                        			<a href="images/item-bw-1.jpg" class="image-popup">
                            			<img src="images/uploaded/2/getresponse-review.jpg" alt="" class="item-img" srcset="">
                        			</a>
                        			<div class="item-title" style="z-index: 10;">
                            			<h4 class="mb-0" style=""><strong class="">Get Response</strong></h4>
                            			<p class="text-secondary mb-0" style="overflow: visible;">Powerful, simplified tool to send emails, create pages, and automate your marketing. Email address sign up free. Start for free | No credit ...<br></p>
                        			</div>
                        			<div class="item-icon"><svg xmlns="http://www.w3.org/2000/svg" height="16" viewBox="0 0 16 16" width="16" class="icon svg-default"><path d="m9 7h7v2h-7v7h-2v-7h-7v-2h7v-7h2z" fill-rule="evenodd"></path></svg></div>
                    			</div><div class="item gallery-item padding dark gallery-style-4">
                        			<a href="images/item-bw-1.jpg" class="image-popup">
                            			<img src="images//uploaded/2/campaignmonitor.jpg" alt="" class="item-img" srcset="">
                        			</a>
                        			<div class="item-title" style="">
                            			<h4 class="mb-0" style=""><strong class="">Campaign Monitor</strong></h4>
                            			<p class="text-secondary mb-0" style="">Drive results with email marketing. Campaign Monitor helps you gain loyal customers and manage customer journeys with best-in-class email campaign software.<br></p>
                        			</div>
                        			<div class="item-icon"><svg xmlns="http://www.w3.org/2000/svg" height="16" viewBox="0 0 16 16" width="16" class="icon svg-default"><path d="m9 7h7v2h-7v7h-2v-7h-7v-2h7v-7h2z" fill-rule="evenodd"></path></svg></div>
                    			</div><div class="item gallery-item padding dark gallery-style-4" data-aos-easing="ease-in-out-back" data-aos="fade-down-left">
                        			<a href="https://vimeo.com/45830194" class="video-popup mfp-iframe">
                            			<img src="images//uploaded/2/MailChimp-Header-How-to-Email-Market.png" alt="" class="item-img" srcset="">
                        			</a>
                        			<div class="item-title">
                            			<h4 class="mb-0 popup-title"><strong>Mailchimp</strong></h4>
                            			<p class="text-secondary mb-0">Grow Your Business With Mailchimp Email Marketing, Forms, Domains, and More. Sign Up Now! Reach New Customers And Take Your Business To The Next Level—Sign Up For Free Today. Automatic Email Solutions. Predicted Demographics. Multi User Account Access.<br></p>
                        			</div>
                        			<div class="item-icon"><svg xmlns="http://www.w3.org/2000/svg" height="16" viewBox="0 0 16 16" width="16" class="icon svg-default"><path d="m9 7h7v2h-7v7h-2v-7h-7v-2h7v-7h2z" fill-rule="evenodd"></path></svg></div>
                    			</div><div class="item gallery-item padding dark gallery-style-4">
                        			<a href="images/item-bw-3.jpg" class="image-popup">
                            			<img src="images//uploaded/2/activecampaign_logo_default.png" alt="" class="item-img" srcset="">
                        			</a>
                        			<div class="item-title" style="">
                            			<h4 class="mb-0" style=""><strong class="">Active Campaign</strong></h4>
                            			<p class="text-secondary mb-0">ActiveCampaign gives you the email marketing, marketing automation, and CRM tools you need to create incredible customer experiences.&nbsp;<br></p>
                        			</div>
                        			<div class="item-icon"><svg xmlns="http://www.w3.org/2000/svg" height="16" viewBox="0 0 16 16" width="16" class="icon svg-default"><path d="m9 7h7v2h-7v7h-2v-7h-7v-2h7v-7h2z" fill-rule="evenodd"></path></svg></div>
                    			</div><div class="item gallery-item padding dark gallery-style-4">
                        			<a href="images/item-bw-4.jpg" class="image-popup">
                            			<img src="images/uploaded/2/aweber-free-email-marketing.d0220352.png" alt="" class="item-img" srcset="">
                        			</a>
                        			<div class="item-title" style="">
                            			<h4 class="mb-0" style=""><strong class="">Aweber</strong></h4>
                            			<p class="text-secondary mb-0" style="">An email marketing solution and so much more at a price you can afford. Check out AWeber's pricing structure which includes a free option.<br></p>
                        			</div>
                        			<div class="item-icon"><svg xmlns="http://www.w3.org/2000/svg" height="16" viewBox="0 0 16 16" width="16" class="icon svg-default"><path d="m9 7h7v2h-7v7h-2v-7h-7v-2h7v-7h2z" fill-rule="evenodd"></path></svg></div>
                    			</div></div>
            			</div>
        			</div>
    			</div>
    			<div class="bg-wrap">
        			<div class="bg"></div>
    			</div>
			</section><section id="contact-form-2" class="pt-50 pb-50 pt-lg-150 pb-lg-150 light">
    			<div class="container">
        			<div class="row">
            			<div class="col-lg-8 ml-auto mr-auto">
                			<div class="content-box padding-x3 bg-default shadow">
                    			<h3 class=""><strong>Contact us</strong></h3>
                    			<p class="mb-30">In our work we try to use only the most modern, convenient and interesting solutions.</p>
                    			<form action="./scripts/request.php" class="contact_form mb-30 form-vertical" id="contact-form-2-form" novalidate="novalidate">
                        			<div class="form-group text-field-group"><input type="text" class="form-control" placeholder="Full name" name="NAME" size="10"></div>
                        			<div class="form-group email-field-group"><input type="email" class="form-control" placeholder="Email Address" name="EMAIL" size="10"></div>
                        			<div class="form-group textarea-group"><textarea class="form-control" placeholder="Your message or question" rows="5" name="MESSAGE"></textarea></div>
                        			<button type="submit" data-loading-text="•••" data-complete-text="Completed!" data-reset-text="Try again later..." class="btn btn-lg btn-dark mt-30">Send message</button>
                    			</form>
                    			<p class="small text-secondary mb-0">*You are very important to us, all information received will always remain confidential.</p>
                			</div>
            			</div>
        			</div>
    			</div>
    			<div class="bg-wrap">
        			<div class="bg"></div>
    			</div>
			</section><footer id="footer-text-logo" class="pt-75 pb-75 text-center dark">
    			<div class="container">
        			<div class="row align-items-center">
           			<div class="col-12">
                			<ul class="list-unstyled list-inline padding-list">
                    			<li class=""><a href="#" class=""><b>Home</b></a></li>
                    			<li class="" style=""><a href="#" class="" style=""><b class="">Showcase</b></a></li>
                    			<li class="" style=""><a href="#" style="" class=""><b class="">Templates</b></a></li>
                    			<li class=""><a href="#" class=""><b>Review</b></a></li>
                    			<li class="" style=""><a href="#" class="" style=""><b class="">Get Started</b></a></li>
                    			<li class="" style=""><a href="#" style="" class=""><b class="">Login</b></a></li>
                    
                			</ul>
            			</div>
            			<div class="col-12">
                			<img class="mw-100 mt-50" src="images//uploaded/2/FP5f833WUAMolMG-removebg-preview.png" height="60px" alt="logo" srcset="">
            			</div>
        			</div>
    			</div>
    			<div class="bg-wrap">
        			<div class="bg"></div>
    			</div>
			</footer>
		</div>
		<footer></footer>
		<div class="modal-container"><div class="modal fade" id="popup-subscribe" tabindex="-1" role="dialog" style="display: none;" aria-hidden="true">
    <div class="modal-dialog" role="document">
        <div class="modal-content padding-x3 text-center light">
            <button type="button" class="close position-absolute r-0 t-0" data-dismiss="modal" aria-label="Close"><svg xmlns="http://www.w3.org/2000/svg" height="16" viewBox="0 0 16 16" width="16" class="icon svg-secondary"><path d="m8.07106781 6.65685425 5.65685429-5.65685425 1.4142135 1.41421356-5.65685423 5.65685425 5.65685423 5.65685429-1.4142135 1.4142135-5.65685429-5.65685423-5.65685425 5.65685423-1.41421356-1.4142135 5.65685425-5.65685429-5.65685425-5.65685425 1.41421356-1.41421356z" fill-rule="evenodd"></path></svg></button>
            <div class="modal-body">
                <svg xmlns="http://www.w3.org/2000/svg" enable-background="new 0 0 64 64" height="64" viewBox="0 0 64 64" width="64" class="mb-30 icon svg-default dark padding-x2 rounded-circle" style="background-color: #90cb30;"><path d="m54 0h-44v21.142l-10 4.193v38.665h64v-38.665l-10-4.193zm-2 2v29.716l-20 12.537-20-12.537v-29.716zm-42 28.462-6.835-4.285 6.835-2.866zm-8 31.538v-34.193l30 18.807 30-18.807v34.193zm58.835-35.823-6.835 4.285v-7.151z"></path><path d="m31.529 31.882.471.251.471-.251c.429-.229 10.529-5.698 10.529-12.882 0-3.42-2.579-6-6-6-2.085 0-3.924 1.068-5 2.687-1.076-1.619-2.915-2.687-5-2.687-3.421 0-6 2.58-6 6 0 7.184 10.1 12.653 10.529 12.882zm-4.529-16.882c2.206 0 4 1.794 4 4h2c0-2.206 1.794-4 4-4 2.317 0 4 1.682 4 4 0 5.203-7.086 9.724-9 10.85-1.914-1.126-9-5.647-9-10.85 0-2.318 1.683-4 4-4z"></path></svg>
                <h3 class="mb-30"><strong>Subscribe popup</strong></h3>
                <form action="scripts/modified.php" class="contact_form form-inline mb-20" id="popup-subscribe-form" novalidate="novalidate">
                    <div class="form-group"><input type="email" class="form-control" placeholder="Email Address" name="EMAIL" size="10" id='mail'></div>
                    <button type="submit" data-loading-text="•••" data-complete-text="Completed!" data-reset-text="Try again later..." class="btn btn-link"><strong>Subscribe</strong></button>
                </form>
                <p class="text-secondary mb-0">We are going to provide you with actual and important for you information without spam or fluff.</p>
            </div>
            <div class="bg-wrap">
                <div class="bg"></div>
            </div>
        </div>
    </div>
</div></div>
		
		
		<script src="https://maps.googleapis.com/maps/api/js?key="></script>
		<script src="https://cdnjs.cloudflare.com/ajax/libs/popper.js/1.11.0/umd/popper.min.js"></script>
		<script src="js/jquery-2.1.4.min.js"></script>
		<script src="js/bootstrap.min.js"></script>
		<script src="js/jquery.vide.min.js"></script>
		<script src="js/owl.carousel.js"></script>
		<script src="js/jquery.magnific-popup.min.js"></script>
		<script src="js/jquery.validate.min.js"></script>
		<script src="js/jquery.smooth-scroll.min.js"></script>
		<script src="js/rellax.min.js"></script>
		<script src="js/aos.js"></script>
		<script src="js/custom.js"></script>
		<script src="js/n.js"></script>
	</body>
</html>